...
//
// SCOPE (decision 2026-08-31, DECISIONS.md): this is the CONSTANT-covariance
// form -- all of (F, Q, A, Z, R) fixed over t -- which is everything S2's
// no-SV model needs. S3 generalizes this same function to time-varying
// covariance (spec §2.2's full contract: SV makes the IS/PC shock scales,
// i.e. the measurement covariance R_t here, time-varying) and must include a
// regression check that the constant case still reproduces S2's G1/G5a
...
//   yobs_t = A' x_t + Z xi_t + e_t,   e_t ~ N(0, R)     (obs m, exog k)
//
...
/**
 * Kalman-filter log-likelihood, constant system matrices.
 *
 ...
 * @param R    m x m measurement covariance
