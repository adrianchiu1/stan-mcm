...
| Time-varying `Z_t` (measurement loadings) | **Built + validated (S9)**: `array[] matrix Z` core with seven delegating constant overloads, Python mirror `_as_Z_path` (filter, RTS inputs, DK plus path, shock recovery); G1 at ≤7.3e-12 over seven paths (the five S8 paths + `loglik_tvz` + the full `Z_t`/`Q_t`/`R_t` core); the constant-Z Stan values reproduce the pre-S9 program EXACTLY on all five S8 paths (fixture pin) | No | **Authored E4** (done): a state multiplied by a lagged observable / exogenous series -- the TVP regression, TVP-AR(1)-SV, TVP-VAR | Done by the `R_t`/`Q_t` playbook; the run-hash consequence taken once more (DECISIONS.md 2026-09-05). `A_t` (a time-varying exogenous map) is still not built: a product of a state and a regressor IS the `Z_t` route, so no family has needed it. |
| Time-varying transition `F_t` / drift paths (Primiceri) | **Not built** (a product in a transition equation is rejected as E6) | No | TVP-VAR in the transition (Primiceri 2005) -- S10 E6 | Same playbook: `array[] matrix F` core + overloads + mirror + G1 path; the engine's `propagate_state_shock` becomes time-varying. |
